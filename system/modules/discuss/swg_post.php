...
if (!isset ($direct_settings['discuss_https_post'])) { $direct_settings['discuss_https_post'] = false; }
if (!isset ($direct_settings['discuss_latest_posts_on_page'])) { $direct_settings['discuss_latest_posts_on_page'] = 5; }
if (!isset ($direct_settings['discuss_post_edit_credits_onetime'])) { $direct_settings['discuss_post_edit_credits_onetime'] = 0; }
if (!isset ($direct_settings['discuss_post_edit_without_notice'])) { $direct_settings['discuss_post_edit_without_notice'] = 300; }
if (!isset ($direct_settings['discuss_post_min'])) { $direct_settings['discuss_post_min'] = 12; }
if (!isset ($direct_settings['discuss_post_new_credits_onetime'])) { $direct_settings['discuss_post_new_credits_onetime'] = 0; }
if (!isset ($direct_settings['discuss_post_new_credits_periodically'])) { $direct_settings['discuss_post_new_credits_periodically'] = 0; }
...
case "edit-save":
{
	...
	$g_pid = (isset ($direct_settings['dsd']['dpid']) ? ($direct_classes['basic_functions']->inputfilter_basic ($direct_settings['dsd']['dpid'])) : "");
	...
	$g_source_url = ($g_source ? base64_decode ($g_source) : "m=discuss&a=jump&dsd={$g_did_dsd}idata+p;".$g_pid);
	...
	$g_back_link = (((!$g_source)&&($g_connector_url)) ? preg_replace (array ("#\[a\]#","#\[oid\]#","#\[(.*?)\]#"),(array ("jump","idata+p;{$g_pid}++","")),$g_connector_url) : str_replace ("[oid]","dpid+{$g_pid}++",$g_source_url));

	if ($g_mode_save)
	{
		$direct_cachedata['page_this'] = "";
		$direct_cachedata['page_backlink'] = "m=discuss&s=post&a=edit&dsd={$g_did_dsd}dpid+{$g_pid}++connector+".(urlencode ($g_connector))."++source+".(urlencode ($g_source))."++target+".(urlencode ($g_target));
		$direct_cachedata['page_homelink'] = $g_back_link;
	}
	else
	{
		$direct_cachedata['page_this'] = "m=discuss&s=post&a=edit&dsd={$g_did_dsd}dpid+{$g_pid}++connector+".(urlencode ($g_connector))."++source+".(urlencode ($g_source))."++target+".(urlencode ($g_target));
	...
	$g_post_array = ($g_post_object ? $g_post_object->get ($g_pid) : NULL);
	...
	if (is_array ($g_topic_array))
	{
		if (($g_topic_array['ddbdatalinker_sid'] == "cb41ecf6e90a594dcea60b6140251d62")&&($g_topic_array['ddbdatalinker_id_main']))
		{
			...
			if ((!$g_post_object->is_writable ())&&((!$g_post_object->is_writable_group ())||(!$direct_classes['kernel']->v_group_user_check_group ("discuss_".$g_board_array['ddbdatalinker_id_object']."_writable")))) { $g_rights_check = false; }
		}
		elseif ($g_post_object->is_writable ())
		{
	...
	elseif ($g_rights_check)
	{
		if ($g_mode_save)
		{
			if ($g_datasub_check) { direct_output_related_manager ("discuss_post_edit_{$g_pid}_form_save","pre_module_service_action"); }
			else { direct_output_related_manager ("discuss_post_edit_{$g_board_array['ddbdatalinker_id']}_{$g_topic_array['ddbdatalinker_id']}_{$g_pid}_form_save","pre_module_service_action"); }
		}
		elseif ($g_datasub_check) { direct_output_related_manager ("discuss_post_edit_{$g_pid}_form","pre_module_service_action"); }
		else { direct_output_related_manager ("discuss_post_edit_{$g_board_array['ddbdatalinker_id']}_{$g_topic_array['ddbdatalinker_id']}_{$g_pid}_form","pre_module_service_action"); }

		if (!$g_mode_save) { $direct_classes['kernel']->service_https ($direct_settings['discuss_https_post'],$direct_cachedata['page_this']); }
		...
		if (!$g_datasub_check) { direct_credits_payment_get_specials ("discuss_post_edit",$g_board_array['ddbdatalinker_id_object'],$direct_settings['discuss_post_edit_credits_onetime'],$g_credits_periodically); }
		$direct_cachedata['output_credits_information'] = direct_credits_payment_info ($direct_settings['discuss_post_edit_credits_onetime'],0);
		$direct_cachedata['output_credits_payment_data'] = direct_credits_payment_check (true,$direct_settings['discuss_post_edit_credits_onetime']);
...
------------------------------------------------------------------------- */
			...
			elseif ((!$g_datasub_check)&&($g_post_object->is_sub_allowed ()))
			{
				$direct_cachedata['i_dsubs_allowed'] = (isset ($GLOBALS['i_dsubs_allowed']) ? (str_replace ("'","",$GLOBALS['i_dsubs_allowed'])) : "");
				$direct_cachedata['i_dsubs_allowed'] = str_replace ("<value value='$direct_cachedata[i_dsubs_allowed]' />","<value value='$direct_cachedata[i_dsubs_allowed]' /><selected value='1' />","<evars><yes><value value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>");
...
"uuid" => $direct_settings['uuid']
				...
				$g_symbol_marked_object = ($g_post_array['ddbdatalinker_symbol'] ? new direct_datacenter () : NULL);
				$g_symbol_marked_array = ($g_symbol_marked_object ? $g_symbol_marked_object->get_aid ("ddbdatacenter_plocation",$g_post_array['ddbdatalinker_symbol']) : NULL);
			...
			if ($g_rights_check)
			{
				if ($g_post_array['ddbdatalinker_datasubs_new']) { $direct_cachedata['i_dsubs_allowed'] = "<evars><yes><value value='1' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }
				...
				if ($g_post_array['ddbdatalinker_datasubs_hide']) { $direct_cachedata['i_dsubs_hidden'] = "<evars><yes><value value='1' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }
				...
				$direct_cachedata['i_dsubs_type'] = (isset ($g_post_array['ddbdatalinker_datasubs_type']) ? str_replace ("'","",$g_post_array['ddbdatalinker_datasubs_type']) : 0);

				$direct_cachedata['i_dgright'] = str_replace ("'","",$g_post_array['ddbdata_mode_group']);
				$direct_cachedata['i_dpright'] = str_replace ("'","",$g_post_array['ddbdata_mode_all']);

				if ($g_post_object->is_locked ()) { $direct_cachedata['i_dlocked'] = "<evars><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no><yes><value value='1' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes></evars>"; }
...
------------------------------------------------------------------------- */
		...
		if ($g_rights_check)
		{
			...
			$direct_classes['formbuilder']->entry_add ("spacer");
			$direct_classes['formbuilder']->entry_add_select ("dlocked",(direct_local_get ("discuss_post_lock")),false,"s");
		}
		...
		$direct_cachedata['output_formtarget'] = "m=discuss&s=post&a=edit-save&dsd={$g_did_dsd}dpid+{$g_pid}++connector+".(urlencode ($g_connector))."++source+".(urlencode ($g_source))."++target+".(urlencode ($g_target));
		$direct_cachedata['output_formtitle'] = direct_local_get ("discuss_post_edit");
...
------------------------------------------------------------------------- */
			...
			if (direct_credits_payment_check (false,$direct_settings['discuss_post_edit_credits_onetime']))
			{
				$direct_cachedata['i_dpost'] = $direct_classes['formtags']->encode ($direct_cachedata['i_dpost']);
				if (($g_post_array['ddbdatalinker_sorting_date'] + $direct_settings['discuss_post_edit_without_notice']) < $direct_cachedata['core_time']) { $direct_cachedata['i_dpost'] .= "\n[rewrite:edit:$direct_cachedata[core_time]:{$direct_settings['user']['id']}:{$direct_settings['user']['name_html']}]"; }
				...
				if ($direct_cachedata['i_dpreview'])
				{
					...
					$direct_cachedata['output_preview_function'] = "oset_discuss_post_preview";

					if ($g_datasub_check) { direct_output_related_manager ("discuss_post_edit_{$g_pid}_form_save","post_module_service_action"); }
					else { direct_output_related_manager ("discuss_post_edit_{$g_board_array['ddbdatalinker_id']}_{$g_topic_array['ddbdatalinker_id']}_{$g_pid}_form_save","post_module_service_action"); }
				...
				if ($g_continue_check)
				{
					...
					$g_post_array['ddbdatalinker_symbol'] = $direct_cachedata['i_dsymbol'];
					$g_post_array['ddbdatalinker_title'] = $direct_cachedata['i_dtitle'];

					if (isset ($direct_cachedata['i_dsubs_allowed']))
					{
						if ($direct_cachedata['i_dsubs_allowed'])
						{
							$g_post_array['ddbdatalinker_datasubs_type'] = $direct_cachedata['i_dsubs_type'];
							$g_post_array['ddbdatalinker_datasubs_hide'] = $direct_cachedata['i_dsubs_hidden'];
							$g_post_array['ddbdatalinker_datasubs_new'] = 1;
						}
						else { $g_post_array['ddbdatalinker_datasubs_new'] = 0; }
					}

					$g_post_array['ddbdiscuss_posts_locked'] = $direct_cachedata['i_dlocked'];
					$g_post_array['ddbdata_data'] = $direct_cachedata['i_dpost'];

					if ($g_post_object->set_update ($g_post_array))
					{
						direct_credits_payment_exec ("discuss","post_edit",$g_pid,$direct_settings['user']['id'],$direct_settings['discuss_post_edit_credits_onetime'],0);

						$direct_cachedata['output_job'] = direct_local_get ("discuss_post_edit");
						$direct_cachedata['output_job_desc'] = direct_local_get ("discuss_done_post_edit");

						if ($g_target_url)
						{
							$direct_cachedata['output_jsjump'] = 2000;
							$g_target_link = str_replace ("[oid]","dpid_d+{$g_pid}++",$g_target_url);
						}
						elseif ($g_connector_url)
						{
							$direct_cachedata['output_jsjump'] = 2000;
							$g_target_link = str_replace (array ("[a]","[oid]"),(array ("jump","idata+p;{$g_pid}++")),$g_connector_url);
						}
			...
			else { $direct_classes['error_functions']->error_page ("standard","core_credits_insufficient","SERVICE ERROR:<br />".(-1 * $direct_settings['discuss_post_edit_credits_onetime'])." Credits are required but not available. This error has been reported by the sWG Credits Manager.<br />sWG/#echo(__FILEPATH__)# _a=edit-save_ (#echo(__LINE__)#)"); }
		}
		else
		{
/* -------------------------------------------------------------------------
...
------------------------------------------------------------------------- */
			...
			if ($g_datasub_check) { direct_output_related_manager ("discuss_post_edit_{$g_pid}_form","post_module_service_action"); }
			else { direct_output_related_manager ("discuss_post_edit_{$g_board_array['ddbdatalinker_id']}_{$g_topic_array['ddbdatalinker_id']}_{$g_pid}_form","post_module_service_action"); }
...
case "reply":
{
	...
	$g_did = (isset ($direct_settings['dsd']['ddid']) ? ($direct_classes['basic_functions']->inputfilter_basic ($direct_settings['dsd']['ddid'])) : "");
	$g_reply_post_id = (isset ($direct_settings['dsd']['dpid']) ? ($direct_classes['basic_functions']->inputfilter_basic ($direct_settings['dsd']['dpid'])) : "");
	...
	$g_did_dsd = (strlen ($g_did) ? "ddid+{$g_did}++" : "");
	$g_pid_dsd = (strlen ($g_reply_post_id) ? "dpid+{$g_reply_post_id}++" : "");
	$g_connector_url = ($g_connector ? base64_decode ($g_connector) : "m=discuss&a=[a]&dsd={$g_did_dsd}[oid]");
	...
	{
		$direct_cachedata['page_this'] = "";
		$direct_cachedata['page_backlink'] = "m=discuss&s=post&a=new&dsd={$g_did_dsd}{$g_pid_dsd}dtid+{$g_tid}++connector+".(urlencode ($g_connector))."++source+".(urlencode ($g_source))."++target+".(urlencode ($g_target));
		$direct_cachedata['page_homelink'] = $g_back_link;
	}
	else
	{
		$direct_cachedata['page_this'] = "m=discuss&s=post&a=new&dsd={$g_did_dsd}{$g_pid_dsd}dtid+{$g_tid}++connector+".(urlencode ($g_connector))."++source+".(urlencode ($g_source))."++target+".(urlencode ($g_target));
		$direct_cachedata['page_backlink'] = $g_back_link;
		$direct_cachedata['page_homelink'] = $g_back_link;
	}
	...
	if (is_array ($g_topic_array))
	{
		...
		if (($g_topic_array['ddbdatalinker_sid'] == "cb41ecf6e90a594dcea60b6140251d62")&&($g_topic_array['ddbdatalinker_id_main']))
		{
			$g_board_object = new direct_discuss_board ();
			if ($g_board_object) { $g_board_array = $g_board_object->get ($g_topic_array['ddbdatalinker_id_main']); }
			if ((is_array ($g_board_array))&&($g_rights_check)) { $g_rights_check = $g_board_object->is_writable (); }
		}
		else { $g_datasub_parent_check = true; }
	}

	if ((!$g_datasub_parent_check)&&(!is_array ($g_board_array))) { $direct_classes['error_functions']->error_page ("standard","discuss_tid_invalid","sWG/#echo(__FILEPATH__)# _a={$direct_settings['a']}_ (#echo(__LINE__)#)"); }
	elseif ($g_rights_check)
	{
		if ($g_mode_save)
		{
			if ($g_datasub_parent_check) { direct_output_related_manager ("discuss_post_new_{$g_tid}_form_save","pre_module_service_action"); }
			else { direct_output_related_manager ("discuss_post_new_{$g_board_array['ddbdatalinker_id']}_{$g_tid}_form_save","pre_module_service_action"); }
		}
		elseif ($g_datasub_parent_check) { direct_output_related_manager ("discuss_post_new_{$g_tid}_form","pre_module_service_action"); }
		else { direct_output_related_manager ("discuss_post_new_{$g_board_array['ddbdatalinker_id']}_{$g_tid}_form","pre_module_service_action"); }
		...
		if (($g_reply_post_id)||($g_mode_save))
		{
			$direct_classes['basic_functions']->require_file ($direct_settings['path_system']."/classes/dhandler/swg_discuss_post.php");
			$direct_classes['basic_functions']->require_file ($direct_settings['path_system']."/classes/swg_formtags.php");
		}
		...
		direct_class_init ("formbuilder");
		if (($g_reply_post_id)||($g_mode_save)) { direct_class_init ("formtags"); }
		direct_class_init ("output");
		$direct_classes['output']->options_insert (2,"servicemenu",$direct_cachedata['page_backlink'],(direct_local_get ("core_back")),$direct_settings['serviceicon_default_back'],"url0");
		...
		{
			$g_post_object = new direct_discuss_post ();
			$g_post_array = ($g_post_object ? $g_post_object->get ($g_reply_post_id) : NULL);
			if (!is_array ($g_post_array)) { $g_reply_post_id = ""; }
		}

		if (!$g_datasub_parent_check) { direct_credits_payment_get_specials ("discuss_post_new",$g_board_array['ddbdatalinker_id_object'],$direct_settings['discuss_post_new_credits_onetime'],$direct_settings['discuss_post_new_credits_periodically']); }
		$direct_cachedata['output_credits_information'] = direct_credits_payment_info ($direct_settings['discuss_post_new_credits_onetime'],$direct_settings['discuss_post_new_credits_periodically']);
		$direct_cachedata['output_credits_payment_data'] = direct_credits_payment_check (true,$direct_settings['discuss_post_new_credits_onetime']);

		if ($g_datasub_parent_check)
		{
...
------------------------------------------------------------------------- */
		...
		else
		{
			...
			if ($g_reply_post_id)
			{
				...
				$direct_cachedata['i_dtitle'] = "Re: ".(preg_replace ("#^((Re\: )+)#i","",$direct_cachedata['i_dtitle']));

				$direct_cachedata['i_dpost'] = preg_replace ("#\[quote(.*?)\](.*?)\[\/quote\](\[newline\]|)#i","",$g_post_array['ddbdata_data']);
				$direct_cachedata['i_dpost'] = "[quote:{$g_post_array['ddbdiscuss_posts_user_id']}:{$g_post_array['ddbusers_name']}]{$direct_cachedata['i_dpost']}[/quote]";
				$direct_cachedata['i_dpost'] = direct_output_smiley_cleanup ($direct_cachedata['i_dpost']);
				$direct_cachedata['i_dpost'] = (($direct_classes['formtags']->recode_newlines ($direct_cachedata['i_dpost'],false))."\n");
			}
			else
			{
				if ($g_datasub_check) { $direct_cachedata['i_dtitle'] = ""; }
				else { $direct_cachedata['i_dtitle'] = (((isset ($g_topic_array['ddbdatalinker_title_alt']))&&(strlen ($g_topic_array['ddbdatalinker_title_alt']))) ? "Re: ".$g_topic_array['ddbdatalinker_title_alt'] : "Re: ".$g_topic_array['ddbdatalinker_title']); }
...
// md5 ("datacenter")
"datacenter_marker_type" => "files-only",
"datacenter_selection_did" => $direct_settings['discuss_datacenter_symbols_did'],
"datacenter_selection_done" => 0,
"datacenter_selection_path" => $direct_settings['discuss_datacenter_path_symbols'],
"datacenter_selection_quantity" => 1,
"uuid" => $direct_settings['uuid']
);
			...
			if ($g_rights_check)
			{
				if ((!$g_datasub_check)&&($g_topic_array['ddbdatalinker_datasubs_new'])) { $direct_cachedata['i_dsubs_allowed'] = "<evars><yes><value value='1' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }
				else { $direct_cachedata['i_dsubs_allowed'] = "<evars><yes><value value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }

				if (($g_datasub_check)||($g_topic_array['ddbdatalinker_datasubs_hide'])) { $direct_cachedata['i_dsubs_hidden'] = "<evars><yes><value value='1' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }
				else { $direct_cachedata['i_dsubs_hidden'] = "<evars><yes><value value='1' /><text><![CDATA[".(direct_local_get ("core_yes"))."]]></text></yes><no><value value='0' /><selected value='1' /><text><![CDATA[".(direct_local_get ("core_no"))."]]></text></no></evars>"; }

				$direct_cachedata['i_dsubs_type'] = (((!$g_datasub_check)&&(isset ($g_topic_array['ddbdatalinker_datasubs_type']))) ? str_replace ("'","",$g_topic_array['ddbdatalinker_datasubs_type']) : 0);

				$direct_cachedata['i_dgright'] = ((($g_datasub_parent_check)||($g_board_array['ddbdiscuss_boards_public'])) ? "-" : "r");
				$direct_cachedata['i_dpright'] = ((($g_datasub_parent_check)||($g_board_array['ddbdiscuss_boards_public'])) ? "r" : "-");
			}
			elseif ((!$g_datasub_check)&&($g_topic_object->is_sub_allowed ()))
			{
...
------------------------------------------------------------------------- */
		...
		if (($direct_settings['discuss_account_status_ex'])&&(!$direct_classes['kernel']->v_usertype_get_int ($direct_settings['user']['type']))) { $direct_classes['formbuilder']->entry_add_embed ("dstatus_ex",NULL,true,"m=dataport&s=swgap;account;status_ex&dsd=",true,"s"); }
		$direct_classes['formbuilder']->entry_add_text ("dtitle",(direct_local_get ("discuss_topic")),false,"m",$direct_settings['discuss_topic_min'],$direct_settings['discuss_topic_max']);
		...
		$direct_cachedata['output_formbutton'] = direct_local_get ("core_save");
		$direct_cachedata['output_formelements'] = $direct_classes['formbuilder']->form_get ($g_mode_save);
		$direct_cachedata['output_formiview_id'] = $g_tid;
		$direct_cachedata['output_formiview_title'] = direct_local_get ("discuss_post_list_latest");
		$direct_cachedata['output_formiview_url'] = "m=dataport&s=swgap;datalinker;iview&a=list_mobjects&dsd=deid+{$g_tid}++decount+{$direct_settings['discuss_latest_posts_on_page']}++source+".(urlencode ($g_source));
		$direct_cachedata['output_formtarget'] = "m=discuss&s=post&a=new-save&dsd={$g_did_dsd}{$g_pid_dsd}dtid+{$g_tid}++connector+".(urlencode ($g_connector))."++source+".(urlencode ($g_source))."++target+".(urlencode ($g_target));
		$direct_cachedata['output_formtitle'] = direct_local_get ("discuss_post_new");
...
------------------------------------------------------------------------- */
			...
			if (!isset ($direct_cachedata['i_dgright'])) { $direct_cachedata['i_dgright'] = ($g_datasub_parent_check ? "-" : "r"); }
			if (!isset ($direct_cachedata['i_dpright'])) { $direct_cachedata['i_dpright'] = ((($g_datasub_parent_check)||($g_board_array['ddbdiscuss_boards_public'])) ? "r" : "-"); }

			if (direct_credits_payment_check (false,$direct_settings['discuss_post_new_credits_onetime']))
			{
				$direct_cachedata['i_dpost'] = $direct_classes['formtags']->encode ($direct_cachedata['i_dpost']);
				...
				if ($direct_cachedata['i_dpreview'])
				{
					...
					if ($g_datasub_parent_check) { direct_output_related_manager ("discuss_post_new_{$g_tid}_form_save","post_module_service_action"); }
					else { direct_output_related_manager ("discuss_post_new_{$g_board_array['ddbdatalinker_id']}_{$g_tid}_form_save","post_module_service_action"); }
				...
				else
				{
					...
					{
						$direct_classes['error_functions']->error_page ("login","account_status_ex_login_first","sWG/#echo(__FILEPATH__)# _a=new-save_ (#echo(__LINE__)#)");
						$g_continue_check = false;
					}
				}
...
"ddbdata_mode_all" => $direct_cachedata['i_dpright']
					...
					if ($g_datasub_check) { $g_insert_array['ddbdatalinker_id_parent'] = $g_tid; }
					else
					{
						$g_insert_array['ddbdatalinker_id_parent'] = $g_reply_post_id;
						$g_insert_array['ddbdatalinker_id_main'] = $g_topic_array['ddbdatalinker_id'];
					}
					...
					{
						$g_insert_array['ddbdatalinker_datasubs_type'] = $g_topic_array['ddbdatalinker_datasubs_type'];
						$g_insert_array['ddbdatalinker_datasubs_hide'] = $g_topic_array['ddbdatalinker_datasubs_hide'];
						$g_insert_array['ddbdatalinker_datasubs_new'] = $g_topic_array['ddbdatalinker_datasubs_new'];
					}

					$g_continue_check = false;
					$g_post_object = new direct_discuss_post ();
					if ($g_post_object) { $g_continue_check = $g_post_object->set_insert ($g_insert_array); }

					if ((!$g_datasub_check)&&($g_continue_check))
					{
						$g_topic_array['ddbdatalinker_objects'] += 1;
						$g_topic_array['ddbdatalinker_sorting_date'] = $direct_cachedata['core_time'];
						$g_topic_array['ddbdiscuss_topics_last_id'] = $direct_settings['user']['id'];
						$g_topic_array['ddbdiscuss_topics_last_ip'] = $direct_settings['user_ip'];
						$g_topic_array['ddbdiscuss_topics_last_preview'] = $g_post_preview;
						$g_continue_check = $g_topic_object->set_update ($g_topic_array);
					}

					if ((!$g_datasub_parent_check)&&($g_continue_check))
					{
						$g_board_array['ddbdatalinker_sorting_date'] = $direct_cachedata['core_time'];
						$g_board_array['ddbdiscuss_boards_posts'] += 1;
						$g_board_array['ddbdiscuss_boards_last_tid'] = $g_tid;
					...
					{
						direct_credits_payment_exec ("discuss","post_new",$g_post_id,$direct_settings['user']['id'],$direct_settings['discuss_post_new_credits_onetime'],$direct_settings['discuss_post_new_credits_periodically']);

						$direct_cachedata['output_job'] = direct_local_get ("discuss_post_new");
						$direct_cachedata['output_job_desc'] = direct_local_get ("discuss_done_post_new");

						if ($g_target_url)
						{
							$direct_cachedata['output_jsjump'] = 2000;
							$g_target_link = str_replace ("[oid]","dpid_d+{$g_post_id}++",$g_target_url);
						}
						elseif ($g_connector_url)
						{
							$direct_cachedata['output_jsjump'] = 2000;
							$g_target_link = str_replace (array ("[a]","[oid]"),(array ("jump","idata+p;{$g_post_id}++")),$g_connector_url);
						}
						else { $direct_cachedata['output_jsjump'] = 0; }
					...
					else
					{
						...
						$direct_classes['error_functions']->error_page ("fatal","core_database_error","sWG/#echo(__FILEPATH__)# _a=new-save_ (#echo(__LINE__)#)");
					}
				}
			}
			else { $direct_classes['error_functions']->error_page ("standard","core_credits_insufficient","SERVICE ERROR:<br />".(-1 * $direct_settings['discuss_post_new_credits_onetime'])." Credits are required but not available. This error has been reported by the sWG Credits Manager.<br />sWG/#echo(__FILEPATH__)# _a=new-save_ (#echo(__LINE__)#)"); }
		}
		else
		{
/* -------------------------------------------------------------------------
...
------------------------------------------------------------------------- */
			...
			{
				$direct_cachedata['output_title'] = direct_html_encode_special ($direct_cachedata['i_dtitle']);
				$direct_cachedata['output_post'] = $direct_classes['formtags']->decode (direct_output_smiley_encode ($direct_classes['formtags']->encode ($direct_cachedata['i_dpost'])));
				$direct_cachedata['output_username'] = $direct_settings['user']['name_html'];
			...
			if ($g_datasub_parent_check) { direct_output_related_manager ("discuss_post_new_{$g_tid}_form","post_module_service_action"); }
			else { direct_output_related_manager ("discuss_post_new_{$g_board_array['ddbdatalinker_id']}_{$g_tid}_form","post_module_service_action"); }

			if ($g_reply_post_id) { $direct_classes['output']->oset ("default","form_preview"); }
			else { $direct_classes['output']->oset ("default","form"); }
...
case "state":
{
	...
	$direct_cachedata['page_this'] = "m=discuss&s=post&a=state&dsd={$g_did_dsd}dpid+{$g_pid}++dchange+{$g_change_type}++connector+".(urlencode ($g_connector))."++source+".(urlencode ($g_source))."++target+".(urlencode ($g_target));
	$direct_cachedata['page_backlink'] = $g_back_link;
	$direct_cachedata['page_homelink'] = $g_back_link;
	...
	if ((is_array ($g_post_array))&&($g_post_array['ddbdatalinker_id_main']))
	{
		$g_topic_object = new direct_discuss_topic ();
		if ($g_topic_object) { $g_topic_array = $g_topic_object->get ($g_post_array['ddbdatalinker_id_main']); }
	}
	...
	if ($g_rights_check)
	{
		if ($g_datasub_check) { direct_output_related_manager ("discuss_post_state_".$g_pid,"pre_module_service_action"); }
		else { direct_output_related_manager ("discuss_post_state_{$g_topic_array['ddbdatalinker_id_main']}_{$g_topic_array['ddbdatalinker_id']}_".$g_pid,"pre_module_service_action"); }
		...
		case "lock":
		{
			...
			$g_continue_check = $g_post_object->define_lock (true,true);
		...
		case "unlock":
		{
			...
			$g_continue_check = ($g_post_object->define_lock (false,true) ? false : true);
