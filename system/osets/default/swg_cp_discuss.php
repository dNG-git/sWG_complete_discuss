...
NOTE_END //n*/
/**
* osets/default/swg_cp_discuss.php
*
...
$f_return = ("<p class='pagecontenttitle'>".(direct_local_get ("cp_discuss_boards"))."</p><script language='JavaScript1.5' type='text/javascript'><![CDATA[
...
function djs_cp_discuss_board_list_{$direct_cachedata['output_did']}_move_response () { djs_swgAJAX_response ('cp_discuss_board_list_{$direct_cachedata['output_did']}_point','swgAJAX_cp_discuss_board_list_{$direct_cachedata['output_did']}_point',''); }
	...
	if ((isset ($f_subs_array['data']))&&(!empty ($f_subs_array['data'])))
	{
		...
		{
$f_return .= (" <span style='font-size:10px'>(<a id='swgAJAX_cp_discuss_board_list_{$direct_cachedata['output_did']}_{$f_subs_array['data']['oid']}_move_up_point' href=\"{$f_subs_array['data']['pageurl_up']}\" target='_self'>".(direct_local_get ("cp_discuss_board_up"))."</a> - <a id='swgAJAX_cp_discuss_board_list_{$direct_cachedata['output_did']}_{$f_subs_array['data']['oid']}_move_down_point' href=\"{$f_subs_array['data']['pageurl_down']}\" target='_self'>".(direct_local_get ("cp_discuss_board_down"))."</a><script language='JavaScript1.5' type='text/javascript'><![CDATA[
...
/**
* direct_output_oset_cp_discuss_boards ()
*
* @uses   direct_debug()
* @uses   USE_debug_reporting
* @return string Valid XHTML code
...
function direct_output_oset_cp_discuss_boards ()
{
	...
	$f_return = "<p class='pagecontenttitle'>".(direct_local_get ("cp_discuss_boards"))."</p>";

	if (empty ($direct_cachedata['output_boards'])) { $f_return .= "\n<p class='pagecontent' style='font-weight:bold'>".(direct_local_get ("cp_discuss_board_list_empty"))."</p>"; }
	else
	{
